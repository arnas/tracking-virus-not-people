...
const parse = async (file) => {
    ...
    if (chosenFiles) {
        const parsedData = await Promise.all([readZipFile(chosenFiles[0]), readZipFile(chosenFiles[1])]);
...
const processFilesData = (files) => {
    ...
    return merged.filter((obj) => obj.hasOwnProperty('placeVisit') && (obj.placeVisit.placeConfidence === "HIGH_CONFIDENCE" || obj.placeVisit.placeConfidence === "MEDIUM_CONFIDENCE"))
        .map((obj) => ({
